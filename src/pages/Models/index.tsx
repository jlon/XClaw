  ...
  return (
    <WorkspacePageFrame>
      <WorkspacePageShell>
        ...
        {/* Content Area */}
        <WorkspacePageScrollArea className="space-y-8">
          {/* AI Providers Section */}
          <ProvidersSettings />

          {/* Token Usage History Section */}
          <section className={usageSurfaceClass}>
            <h2 className="mb-6 text-[28px] font-semibold tracking-tight text-foreground">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <div className={toggleGroupClass}>
                        <Button
                          ...
                          className={usageGroupBy === 'model' ? toggleActiveClass : toggleIdleClass}
                        >
                        ...
                        <Button
                          ...
                          className={usageGroupBy === 'day' ? toggleActiveClass : toggleIdleClass}
                        >
                      ...
                      <div className={toggleGroupClass}>
                        <Button
                          ...
                          className={usageWindow === '7d' ? toggleActiveClass : toggleIdleClass}
                        >
                        ...
                        <Button
                          ...
                          className={usageWindow === '30d' ? toggleActiveClass : toggleIdleClass}
                        >
                        ...
                        <Button
                          ...
                          className={usageWindow === 'all' ? toggleActiveClass : toggleIdleClass}
                        >
                      ...
                      >
                        ...
                        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5 text-[12.5px] font-medium text-muted-foreground">
                          ...
                          {entry.cacheReadTokens > 0 && (
                            <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-[hsl(var(--warning))]" />{t('dashboard:recentTokenHistory.cacheRead', { value: formatTokenCount(entry.cacheReadTokens) })}</span>
                          )}
                          {entry.cacheWriteTokens > 0 && (
                            <span className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-[hsl(var(--warning))]" />{t('dashboard:recentTokenHistory.cacheWrite', { value: formatTokenCount(entry.cacheWriteTokens) })}</span>
                          )}
                          {typeof entry.costUsd === 'number' && Number.isFinite(entry.costUsd) && (
                            <span className={cn('ml-auto flex items-center gap-1.5', usageChipClass)}>{t('dashboard:recentTokenHistory.cost', { amount: entry.costUsd.toFixed(4) })}</span>
                          )}
...
function UsageBarChart({
  ...
  if (groups.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border/70 bg-[hsl(var(--surface-base)/0.72)] p-8 text-center text-[14px] font-medium text-muted-foreground">
        {emptyLabel}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-border/70 bg-[hsl(var(--surface-base)/0.84)] p-5">
      <div className="flex flex-wrap gap-4 text-[13px] font-medium text-muted-foreground mb-2">
        <span className="inline-flex items-center gap-2">
          <span className="h-2.5 w-2.5 rounded-full bg-[hsl(var(--info))]" />
          {inputLabel}
        </span>
        <span className="inline-flex items-center gap-2">
      ...
      {groups.map((group) => (
        <div key={group.label} className="space-y-1.5">
          ...
          <div className="h-3.5 overflow-hidden rounded-full bg-[hsl(var(--surface-hover)/0.92)]">
            <div
