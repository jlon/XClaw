...
所有验收项都必须能映射回 `WU-01` 到 `WU-08`；如果涉及动画，还必须额外映射到 `WM-01` 到 `WM-06`。
...
- `WM-01`：导航反馈只过渡状态，不做 scale/bounce
- `WM-02`：资源卡 hover 的位移统一为 `-1px`
- `WM-03`：工具带控件不再使用 `transition-all`
- `WM-05`：`prefers-reduced-motion` 下不再发生 hover 上浮
...
- `WM-01`：导航、列表、按钮的过渡时长应回到统一 token，不再出现散落的 `150 / 180 / 200ms`
- `WM-02`：卡片 selected 与 hover 必须维持清楚层级，hover 不得重于 selected
...
- 侧边栏、资源卡、工具带 hover 手感一致，不再一页只变色、一页又上浮、一页 `transition-all`
...
- 动画存在感低，但切换和 hover 手感一致，不再出现网页式“各动各的”
