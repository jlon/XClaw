...
第七轮欢迎态收口已完成。欢迎区已经从过渡性的建议卡改成 4 张 OpenClaw 工作模式卡，中轴主视觉也已从“近似图标”升级为按线上 OpenClaw Hero 结构与动画参数还原、并追加低频桌面化 special-idle 的动态小龙虾资产；本轮进一步把欢迎头从页内粗描边字形彻底切换为品牌资产逻辑：`Claw` 直接基于 `QClaw` 欢迎头参考几何，`X` 单独按同一语义重做，最终输出为可跟随主题色的 `mask` 字标。欢迎态定向测试、ESLint 与全量 `typecheck` 已通过。
...
27. 已将欢迎头字标从内联粗描边 helper 拆到独立组件文件，避免继续在聊天页大文件里堆品牌路径。
28. 已确认“继续手工描 `SVG`”是错误路线，并以 `QClaw` 真参考图重新建立字标基线。
29. 已将欢迎头字标切换为 `mask` 资产，`Claw` 直接使用参考几何，`X` 单独按同家族重量与圆角语义重做。
30. 已把标题容器宽度和投影继续收回到更接近 `QClaw` 品牌图的紧凑节奏。
31. 已按 alpha 边界重新裁正字标资产，去掉错误的右侧透明留白。
32. 已把欢迎头容器改为 `min(100%, clamp(...)) + 正确纵横比`，保证页面缩放与窗口变化下仍保持视觉居中。
