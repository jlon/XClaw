...
# Post-installation script for XClaw on Linux
...
# Create symbolic link for XClaw app binary
if [ -x /opt/XClaw/XClaw ]; then
    ln -sf /opt/XClaw/XClaw /usr/local/bin/XClaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/XClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/XClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/XClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/XClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/XClaw'
...
echo "XClaw has been installed successfully."
