...
DATA_DIR = os.path.abspath(os.path.expanduser(os.getenv("STAR_OFFICE_DATA_DIR") or ROOT_DIR))
os.makedirs(DATA_DIR, exist_ok=True)
LAST_KNOWN_GOOD_DIR = os.path.join(DATA_DIR, "last-known-good")
STATE_FILE = os.path.join(DATA_DIR, "state.json")
AGENTS_STATE_FILE = os.path.join(DATA_DIR, "agents-state.json")
JOIN_KEYS_FILE = os.path.join(DATA_DIR, "join-keys.json")
SNAPSHOT_MANIFEST_FILE = os.path.join(DATA_DIR, "manifest.json")
LAST_KNOWN_GOOD_STATE_FILE = os.path.join(LAST_KNOWN_GOOD_DIR, "state.json")
LAST_KNOWN_GOOD_AGENTS_STATE_FILE = os.path.join(LAST_KNOWN_GOOD_DIR, "agents-state.json")
LAST_KNOWN_GOOD_MANIFEST_FILE = os.path.join(LAST_KNOWN_GOOD_DIR, "manifest.json")
...
READONLY_SESSION_KEY = "star_office_readonly"
EMBEDDED_SESSION_KEY = "star_office_embedded"
...
def _sync_runtime_mode_from_request():
    session[EMBEDDED_SESSION_KEY] = _is_truthy(request.args.get("embedded"))
    session[READONLY_SESSION_KEY] = _is_truthy(request.args.get("readonly"))


def _is_readonly_mode():
    return bool(session.get(READONLY_SESSION_KEY))
...
@app.before_request
def block_write_requests_in_readonly_mode():
    if request.method in {"POST", "PUT", "PATCH", "DELETE"} and _is_readonly_mode():
        return jsonify({"ok": False, "code": "READONLY", "msg": "readonly mode"}), 403
...
def load_state(persist_auto_idle=True):
...
def index():
    ...
    _maybe_apply_random_home_favorite(allow_write=not _is_readonly_mode())
...
def _save_home_favorites_index(data):
    _ensure_home_favorites_index()
    _ensure_parent_dir(HOME_FAVORITES_INDEX_FILE)
    with open(HOME_FAVORITES_INDEX_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def _maybe_apply_random_home_favorite(allow_write=True):
...
def _validate_snapshot_manifest(manifest):
    if not isinstance(manifest, dict):
        return None
    if _as_int(manifest.get("schemaVersion")) != 1:
        return None
    generation = _as_int(manifest.get("generation"))
    if generation is None:
        return None
    if manifest.get("owner") != "xclaw-main":
        return None
    committed_at = manifest.get("committedAt")
    if not isinstance(committed_at, str) or not committed_at.strip():
        return None
    return generation


def _validate_snapshot_state(data, generation):
    if not isinstance(data, dict):
        return None
    if _as_int(data.get("schemaVersion")) != 1:
        return None
    if _as_int(data.get("generation")) != generation:
        return None
    if data.get("owner") != "xclaw-main":
        return None
...
def _validate_snapshot_agents(data, generation):
    if not isinstance(data, dict):
        return None
    if _as_int(data.get("schemaVersion")) != 1:
        return None
    ...
    for item in agents:
        ...
        detail_source = item.get("detailSource")
        ...
        out.append({
            ...
            "updated_at": updated_at,
            "updatedAt": updated_at,
            "area": state_to_area(status),
            "source": "snapshot",
            "joinKey": None,
            "authStatus": "approved",
            "authExpiresAt": None,
            "lastPushAt": updated_at,
            "isMain": agent_id == "main",
...
def _load_snapshot_bundle(root_dir):
    manifest = _load_json_file(os.path.join(root_dir, "manifest.json"))
    generation = _validate_snapshot_manifest(manifest)
    if generation is None:
        return None
    files = manifest.get("files") if isinstance(manifest.get("files"), dict) else {}
    state_file = os.path.join(root_dir, files.get("main") or "state.json")
    agents_file = os.path.join(root_dir, files.get("agents") or "agents-state.json")
    state = _validate_snapshot_state(_load_json_file(state_file), generation)
    agents = _validate_snapshot_agents(_load_json_file(agents_file), generation)
    if not state or agents is None:
        return None
    return {
        "generation": generation,
        "manifest": manifest,
...
def load_display_snapshot():
    return _load_snapshot_bundle(DATA_DIR) or _load_snapshot_bundle(LAST_KNOWN_GOOD_DIR)
...
def get_agents():
    ...
    snapshot = load_display_snapshot()
    if snapshot:
        return jsonify(snapshot["agents"])
...
def get_status():
    ...
    snapshot = load_display_snapshot()
    state = snapshot["state"] if snapshot else load_state(persist_auto_idle=not _is_readonly_mode())
