...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜxclawなのか">なぜXClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**XClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、XClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

XClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。XClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
| 複数のAIプロバイダー | 適応型プロバイダーカードと Token Intelligence ワークベンチ |
| スキル/プラグインのインストール | 組み込みのスキルマーケットプレイスと管理機能 |

### OpenClaw内蔵

XClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
メイン入力欄では、OpenClaw 標準のセッション単位モデル上書きを使って、永続的な provider 設定を変更せずに現在のセッションモデルを直接切り替えることもできます。
チャットワークベンチには QClaw に合わせたローカル slash command router と入力中のコマンドメニューも入りました。`/new` と `/reset` は renderer 側で先に捕捉されつつ、共通の Gateway 送信経路を再利用します。`/model`、`/compact`、`/agents`、`/focus`、`/export`、`/usage` のようなコマンドは通常の会話として送られず、先にローカルで処理されます。`/usage` は現在セッションの token 使用量をチャット内に直接表示し、`/focus` と `/export` は引き続きデスクトップ側アクションとして実行されます。
`@agent` で別のエージェントを選ぶと、XClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
XClaw は公式 WeChat チャンネルプラグインも同梱するようになり、GUI の QR ログインだけで WeChat アカウントの追加や再バインドを行えます。`npx` や `openclaw` の手動実行は不要です。Channels ワークベンチでは実際の WeChat アカウント ID を読み取り専用で保持し、QR 再ログインとヘルスガードによる失効リスク通知を提供しますが、自動のキープアライブ送信は行いません。
...
XClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。XClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に XClaw が自動的に起動します。
...
[Releases](https://github.com/jlon/XClaw/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
XClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
XClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、XClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- XClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
...
XClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
pnpm package:mac          # macOS向けにパッケージ化（DMG + ZIP）
pnpm package:mac:local    # ローカル macOS パッケージ化（macOS 12 ホストでは自動で ZIP のみ）
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がXClawをより良くするのに役立ちます。
...
XClawは優れたオープンソースプロジェクトの上に構築されています：
...
XClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に XClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、XClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=jlon/XClaw&type=Date" alt="スター履歴チャート" />
</p>
...
XClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
