...
type GatewayRuntimeRefresher = Pick<GatewayRuntimeController, 'requestRuntimeRefresh'>;

async function scheduleGatewayRefresh(
  gatewayRuntimeController: GatewayRuntimeRefresher | undefined,
  message: string,
  options?: { delayMs?: number; mode?: GatewayRefreshMode },
): Promise<void> {
  if (!gatewayRuntimeController) {
    return;
  }

  logger.info(message);
  await gatewayRuntimeController.requestRuntimeRefresh({
    mode: options?.mode,
...
export async function syncSavedProviderToRuntime(
  config: ProviderConfig,
  apiKey: string | undefined,
  gatewayRuntimeController?: GatewayRuntimeRefresher,
): Promise<void> {
