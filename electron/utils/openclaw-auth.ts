...
import { getOpenClawAgentsDir, getOpenClawConfigPath, getOpenClawExtensionsDir, getOpenClawResolvedDir } from './paths';
import {
  getProviderEnvVar,
...
async function readPluginManifestId(manifestPath: string): Promise<string | null> {
  const manifest = await readJsonFile<{ id?: unknown }>(manifestPath);
  return typeof manifest?.id === 'string' && manifest.id.trim()
    ? manifest.id.trim()
    : null;
}

async function collectPluginIdsFromDirectory(rootDir: string): Promise<Set<string>> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const pluginId = await readPluginManifestId(join(rootDir, entry.name, 'openclaw.plugin.json'));
      if (pluginId) {
...
function extractConfiguredPluginLoadPaths(pluginsObj: Record<string, unknown>): string[] {
  const load = pluginsObj.load;
  if (Array.isArray(load)) {
    return load.filter((value): value is string => typeof value === 'string' && value.trim().length > 0);
  }
...
async function collectConfiguredLoadPluginIds(pluginsObj: Record<string, unknown>): Promise<Set<string>> {
  const pluginIds = new Set<string>();
  for (const pluginPath of extractConfiguredPluginLoadPaths(pluginsObj)) {
    const manifestPath = pluginPath.endsWith('openclaw.plugin.json')
      ? pluginPath
      : join(pluginPath, 'openclaw.plugin.json');
    const pluginId = await readPluginManifestId(manifestPath);
    if (pluginId) {
      pluginIds.add(pluginId);
    }
  }
...
async function collectResolvablePluginIds(pluginsObj: Record<string, unknown>): Promise<Set<string>> {
  const pluginIds = new Set<string>();
  const addAll = (values: Iterable<string>) => {
    for (const value of values) {
      pluginIds.add(value);
    }
  };

  addAll(await collectPluginIdsFromDirectory(join(getOpenClawResolvedDir(), 'extensions')));
  addAll(await collectPluginIdsFromDirectory(getOpenClawExtensionsDir()));
  addAll(await collectConfiguredLoadPluginIds(pluginsObj));
...
export async function removeProviderFromOpenClaw(provider: string): Promise<void> {
  ...
  try {
    await withConfigLock(async () => {
      ...
      const nextAllow = allow.filter((value) => value !== pluginName);
      if (nextAllow.length !== allow.length) {
        const nextPlugins = plugins && typeof plugins === 'object' ? plugins : {};
        (nextPlugins as Record<string, unknown>).allow = nextAllow;
        config.plugins = nextPlugins;
        modified = true;
        console.log(`Removed OpenClaw plugin allow entry: ${pluginName}`);
      }
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      const resolvablePluginIds = await collectResolvablePluginIds(pluginsObj);

      if (Array.isArray(pluginsObj.allow)) {
        const allowArr = pluginsObj.allow as string[];
        const nextAllow = allowArr.filter((pluginId) => resolvablePluginIds.has(pluginId));
        if (nextAllow.length !== allowArr.length) {
      ...
      for (const pluginId of Object.keys(pEntries)) {
        if (pluginId === 'feishu' || resolvablePluginIds.has(pluginId)) {
          continue;
        }
