...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getXClawProviderStore();
 ...
 * Also synchronizes XClaw local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from XClaw UI to stay consistent.
