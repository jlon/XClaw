...
export async function handleProviderRoutes(
  ...
  if (url.pathname === '/api/provider-accounts' && req.method === 'POST') {
    try {
      ...
      await syncSavedProviderToRuntime(providerAccountToConfig(account), body.apiKey, ctx.gatewayRuntimeController);
  ...
  if (url.pathname === '/api/provider-accounts/default' && req.method === 'PUT') {
    try {
      ...
      await syncDefaultProviderToRuntime(body.accountId, ctx.gatewayRuntimeController);
  ...
  if (url.pathname === '/api/providers/default' && req.method === 'PUT') {
    ...
    try {
      ...
      await syncDefaultProviderToRuntime(body.providerId, ctx.gatewayRuntimeController);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/providers' && req.method === 'POST') {
    ...
    try {
      ...
      await syncSavedProviderToRuntime(config, body.apiKey, ctx.gatewayRuntimeController);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname.startsWith('/api/providers/') && req.method === 'PUT') {
    ...
    try {
      ...
      await syncUpdatedProviderToRuntime(nextConfig, body.apiKey, ctx.gatewayRuntimeController);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname.startsWith('/api/providers/') && req.method === 'DELETE') {
    ...
    try {
      ...
      await syncDeletedProviderToRuntime(existing, providerId, ctx.gatewayRuntimeController);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
