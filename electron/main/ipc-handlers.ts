...
import { resolveRuntimeGatewayPort } from '../gateway/runtime-config';
import { ClawHubService, ClawHubSearchParams, ClawHubInstallParams, ClawHubUninstallParams } from '../gateway/clawhub';
import {
  type ProviderConfig,
} from '../utils/secure-storage';
import { getOpenClawStatus, getOpenClawDir, getOpenClawConfigDir, getOpenClawSkillsDir, getOpenClawMediaDir, ensureDir } from '../utils/paths';
...
export function registerIpcHandlers(
  ...
  registerUnifiedRequestHandlers(gatewayManager, gatewayRuntimeController);

  // Host API proxy handlers
  registerHostApiProxyHandlers();

  // Gateway handlers
  registerGatewayHandlers(gatewayManager, gatewayRuntimeController, mainWindow);

  // ClawHub handlers
  registerClawHubHandlers(clawHubService);

  // OpenClaw handlers
  registerOpenClawHandlers(gatewayRuntimeController);

  // Provider handlers
  registerProviderHandlers(gatewayRuntimeController);
  ...
  // Settings handlers
  registerSettingsHandlers(gatewayRuntimeController);
...
function registerUnifiedRequestHandlers(
  ...
  const applyCurrentProxySettings = async () => {
        ...
        case 'provider': {
          ...
          if (request.action === 'save') {
            ...
            try {
              ...
              try {
                await syncSavedProviderToRuntime(config, apiKey, gatewayRuntimeController);
              } catch (err) {
          ...
          if (request.action === 'delete') {
            ...
            try {
              ...
              if (existing?.type) {
                try {
                  await syncDeletedProviderToRuntime(existing, providerId, gatewayRuntimeController);
                } catch (err) {
          ...
          if (request.action === 'updateWithKey') {
            ...
            try {
              ...
              try {
                await syncUpdatedProviderToRuntime(nextConfig, apiKey, gatewayRuntimeController);
              } catch (err) {
          ...
          if (request.action === 'setDefault') {
            ...
            try {
              ...
              if (provider) {
                try {
                  await syncDefaultProviderToRuntime(providerId, gatewayRuntimeController);
                } catch (err) {
        ...
        case 'settings': {
          ...
          if (request.action === 'set') {
            ...
            await gatewayRuntimeController.applySettingsRuntimeEffects({
              ...
              applyProxySettings: isProxyKey(key) ? applyCurrentProxySettings : null,
              applyLaunchAtStartup: isLaunchAtStartupKey(key) ? syncLaunchAtStartupSettingFromStore : null,
            });
            data = { success: true };
            break;
          }
          if (request.action === 'setMany') {
            ...
            await gatewayRuntimeController.applySettingsRuntimeEffects({
              gatewayPort: gatewayPortEntry?.[1],
              applyProxySettings: entries.some(([key]) => isProxyKey(key)) ? applyCurrentProxySettings : null,
              applyLaunchAtStartup: entries.some(([key]) => isLaunchAtStartupKey(key)) ? syncLaunchAtStartupSettingFromStore : null,
            });
            data = { success: true };
            break;
          }
...
function registerGatewayHandlers(
  ...
  ipcMain.handle('gateway:start', async () => {
    try {
      await gatewayRuntimeController.requestStart();
      return { success: true };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  ...
  ipcMain.handle('gateway:stop', async () => {
    try {
      await gatewayRuntimeController.requestStop();
      return { success: true };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  ...
  ipcMain.handle('gateway:restart', async () => {
    try {
      await gatewayRuntimeController.requestRestart();
      return { success: true };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  ...
  ipcMain.handle('gateway:httpProxy', async (_, request: GatewayHttpProxyRequest) => {
    try {
      const port = await resolveRuntimeGatewayPort(gatewayManager.getStatus());
 ...
 */
function registerOpenClawHandlers(gatewayRuntimeController: GatewayRuntimeController): void {
  ...
  const scheduleGatewayChannelRestart = (reason: string): void => {
    ...
    void gatewayRuntimeController.requestRuntimeRefresh({ mode: 'restart' });
  };
  ...
  const scheduleGatewayChannelSaveRefresh = (channelType: string, reason: string): void => {
    ...
    void gatewayRuntimeController.requestRuntimeRefresh({ mode: 'reload' });
  };
 ...
 */
function registerProviderHandlers(gatewayRuntimeController: GatewayRuntimeController): void {
  const providerService = getProviderService();
  ...
  deviceOAuthManager.on('oauth:success', ({ provider, accountId }) => {
    ...
    void gatewayRuntimeController.requestRuntimeRefresh({ mode: 'restart', delayMs: 8000 });
  });
...
function registerSettingsHandlers(gatewayRuntimeController: GatewayRuntimeController): void {
  const applyCurrentProxySettings = async () => {
    const settings = await getAllSettings();
    await applyProxySettings(settings);
  };
